...
app.listen = function () {
	...
	if(storageProvider == 'mongodb'){
		// Start to serve Asset from s3
		this.use(['/assets/:uid/:file_name', '/:lang/assets/:uid/:file_name'], function (req, res, next) {
			var languages = config.get('languages');
			// console.log(" route start ----------------")
			var assetUid = req.params.uid,
				assetFilename = req.params.file_name,
				langCode = req.params.lang;
			for (var m = 0, _m = languages.length; m < _m; m++) {
				var lang = languages[m];
				_url = lang.relative_url_prefix;
				if (_url === '/' + langCode) {
					langCode = lang.code;
				}
			}
				...
				var params;
			if(langCode){
				params = {Bucket: bucketName, Key: langCode+"/"+assetUid+"/"+assetFilename};
			}
			else{
				params = {Bucket: bucketName, Key: "en-us/"+assetUid+"/"+assetFilename};
			}
			console.log("params:---",params)
			s3.getObject(params,function(err,data){
				if (err) {
					console.log("err in s3 get assets : -- ",err)
					return res.status(500).send("Please Publish Your Assets!");
				}
				// Headers
				res.setHeader("Content-disposition", "inline;  filename=" + assetFilename);
